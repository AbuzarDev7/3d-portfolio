...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('Abu zar M Shahi', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Creative FrontEnd Developer / Mern Stack Developer ',
      {
        font: font,
        size: 0.018,
        height: 0,
...
function switchTheme(themeType) {
  if (themeType === 'dark') {
    ...
    // main lights (deep cinematic blue)
    ...
    gsap.to(roomLight, { intensity: 1.35 });
    gsap.to(ambientLight, { intensity: 0.32 });

    // fan light
    gsap.to(fanLight5, { distance: 0.075 });

    // title text (soft cool white)
    ...
    // text lights ON (soft glow)
    gsap.to(pointLight1, { intensity: 0.65 });
    gsap.to(pointLight2, { intensity: 0.65 });
    gsap.to(pointLight3, { intensity: 0.65 });
    gsap.to(pointLight4, { intensity: 0.65 });
    ...
    gsap.to(roomLight, { intensity: 2.35 });
    gsap.to(ambientLight, { intensity: 0.6 });

    // fan light
    gsap.to(fanLight5, { distance: 0.055 });
